...
void f_consultarPaciente(LinkedList *lista) {
    ...
    char termoBusca[50]; // Variável para armazenar o termo de busca de consulta ao paciente
    ListNode *atual;     
 //==================================================================================================//
    ...
    do {
        ...
        switch (opcao) {
            ...
            case '2': { // Consulta por CPF
                ...
                while (atual) { // Percorre a lista
                    if (strcmp(atual->paciente.cpf, termoBusca) == 0) { // Verifica se o CPF é igual ao buscado
                        f_printPaciente(&atual->paciente); // Imprime os dados do paciente encontrado
                        encontrado = 1;
    ...
    } while (opcao != '3'); // Repete enquanto o usuário não escolher retornar ao menu principal
}

void f_atualizarPaciente(LinkedList *lista) {
    // DECLARAÇÃO DE VARIÁVEIS
    int id, idadeAtualizada;
    char cpfAtualizado[15], nomeAtualizado[50], data_registroAtualizada[11], confirmacao;
//==================================================================================================//

    printf("Digite o ID do registro a ser atualizado: \n"); // Pede ao usuário para digitar o ID do paciente a ser atualizado
    scanf("%d", &id);
    ...
    while (atual) { // Enquanto houver pacientes na lista
        if (atual->paciente.id == id) { // Se o ID do paciente for igual ao ID digitado
            ...
            printf("CPF: %s\n", cpfAtualizado[0] == '-' ? atual->paciente.cpf : f_formataCPF(cpfAtualizado));
            printf("Nome: %s\n", nomeAtualizado[0] == '-' ? atual->paciente.nome : nomeAtualizado);
            printf("Idade: %d\n", idadeAtualizada == -1 ? atual->paciente.idade : idadeAtualizada);
            printf("Data de cadastro: %s\n", data_registroAtualizada[0] == '-' ? atual->paciente.data_cadastro : data_registroAtualizada);

            printf("\nConfirma a atualização? (S/N): "); // Pergunta se o usuário confirma a atualização
            scanf(" %c", &confirmacao);
    ...
    while (atual) { // Enquanto houver pacientes na lista
        if (atual->paciente.id == id) { // Se o ID do paciente for igual ao ID digitado
            ...
            if (confirmacao == 'S' || confirmacao == 's') { // Se a confirmação for 'S' ou 's', remove o paciente
                if (anterior) { 
                    // Se houver um nó anterior, o próximo nó do anterior será o próximo nó do nó atual
                    anterior->next = atual->next;
                } else {
                    // Se não houver um nó anterior, o primeiro nó da lista será o próximo nó do nó atual
...
void f_inserirNovoPaciente(LinkedList *lista) {
    // DECLARAÇÃO DE VARIÁVEIS
    char cpf[15], nome[50], data_de_registro[11], opcao[4];
    ...
    id_anterior = maior_id(lista); // Certifique-se de que a função `maior_id` está implementada corretamente
    ...
    if (strcasecmp(opcao, "S") == 0) { // Usa `strcasecmp` para comparar strings ignorando maiúsculas/minúsculas
