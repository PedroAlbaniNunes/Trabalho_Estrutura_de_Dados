...
void f_printPaciente(
    Paciente *paciente) { // Função para imprimir os dados de um paciente
  printf("ID: %d\n", paciente->id); // Imprime o ID do paciente
  printf("CPF: %s\n",
         f_formataCPF(paciente->cpf)); // Imprime o CPF do paciente já formatado
...
void f_consultarPaciente(LinkedList *lista) {
  ...
  do {
    ...
    case '1': { // Consulta por nome
      ...
      int encontrado =
          0; // Flag para verificar se algum paciente foi encontrado

      while (atual) { // Percorre a lista
        if (strstr(atual->paciente.nome,
                   termoBusca)) { // Verifica se o nome contém o termo buscado
          f_printPaciente(
              &atual->paciente); // Imprime os dados do paciente encontrado
          encontrado = 1;
        }
    ...
    case '2': { // Consulta por CPF
      ...
      while (atual) { // Percorre a lista
        if (strcmp(atual->paciente.cpf, termoBusca) ==
...
void f_atualizarPaciente(LinkedList *lista) {
  ...
  while (atual) {                 // Enquanto houver pacientes na lista
    if (atual->paciente.id ==
        id) { // Se o ID do paciente for igual ao ID digitado
      ...
      printf("CPF atual: %s\nDigite o novo CPF: ",
             f_formataCPF(atual->paciente.cpf));
      ...
      if (confirmacao == 'S' ||
          confirmacao == 's') { // Se a confirmação for 'S' ou 's', atualiza os
                                // dados do paciente
        if (cpfAtualizado[0] != '-') { // Se o novo CPF for diferente de '-',
                                       // atualiza o CPF do paciente
          strcpy(atual->paciente.cpf, cpfAtualizado);
        }

        if (nomeAtualizado[0] != '-') { // Se o novo nome for diferente de '-',
                                        // atualiza o nome do paciente
          strcpy(atual->paciente.nome, nomeAtualizado);
        }

        if (idadeAtualizada != -1) { // Se a nova idade for diferente de -1,
                                     // atualiza a idade do paciente
          atual->paciente.idade = idadeAtualizada;
        }

        if (data_registroAtualizada[0] !=
            '-') { // Se a nova data de cadastro for diferente de '-', atualiza
                   // a data de cadastro do paciente
...
void f_removerPaciente(LinkedList *lista) {
  ...
  while (atual) { // Enquanto houver pacientes na lista
    if (atual->paciente.id ==
      ...
      if (confirmacao == 'S' ||
          confirmacao ==
              's') { // Se a confirmação for 'S' ou 's', remove o paciente
        if (anterior) {
...
int maior_id(LinkedList *lista) {
  ...
  while (atual != NULL) { // Percorre a lista encadeada
    if (atual->paciente.id > maior_id) {
      maior_id =
          atual->paciente.id; // Atualiza o maior ID se o ID atual for maior
    }
    atual = atual->next; // Avança para o próximo nó
  }
  return maior_id; // Retorna o maior ID encontrado
}

void f_inserirNovoPaciente(LinkedList *lista) {
  // DECLARAÇÃO DE VARIÁVEIS
  char cpf[11], nome[50], data_de_registro[11], opcao[4];
  int id_atual, id_anterior, idade;
  //==================================================================================================//

  printf("Para inserir um novo registro, digite os valores para os campos CPF "
         "(apenas dígitos), Nome, Idade e Data_Cadastro: \n");

  // Obter o maior ID existente na lista
  id_anterior = maior_id(lista);
  printf("Maior ID encontrado: %d\n", id_anterior);
  ...
  printf("Confirma a inserção do registro acima? (S/N)\n");
  scanf(" %[^\n]", opcao);

  if (strcasecmp(opcao, "S") == 0) { // Usa `strcasecmp` para comparar strings
                                     // ignorando maiúsculas/minúsculas
    Paciente novoPaciente; // Cria um novo paciente com os dados fornecidos
    novoPaciente.id = id_atual;
    ...
    printf("ID antes de adicionar: %d\n", novoPaciente.id);
    printf("CPF antes de adicionar: %s\n", novoPaciente.cpf);
    printf("Nome antes de adicionar: %s\n", novoPaciente.nome);
    printf("Idade antes de adicionar: %d\n", novoPaciente.idade);
    printf("Data de registro antes de adicionar: %s\n", novoPaciente.data_cadastro);

    f_adicionarNaLista(lista,
                       &novoPaciente); // Adiciona o paciente à lista encadeada
